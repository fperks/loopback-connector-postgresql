...
PostgreSQL.prototype._buildWhere = function(model, where) {
  ...
  for (var key in where) {
    ...
    if (expression === null || expression === undefined) {
      ...
      if ((operator === 'inq' || operator === 'nin') && Array.isArray(p.type)) {
        if (operator === 'inq') {
          sqlExp = new ParameterizedSQL(`(${columnName} @> '[${expression.map(e => `"${this.toColumnValue(p, e)}"`).join(',')}]')`, []);
        } else if (operator === 'nin') {
          sqlExp = new ParameterizedSQL(`NOT (${columnName} @> '[${expression.map(e => `"${this.toColumnValue(p, e)}"`).join(',')}]')`, []);
        }
        stmt.merge(sqlExp);
        whereStmts.push(stmt);
        continue;
      } else if (operator === 'inq' || operator === 'nin' || operator === 'between') {
        columnValue = [];
